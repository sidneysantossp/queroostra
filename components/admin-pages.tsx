  ...
  return (
    <AdminShell>
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_340px]">
        ...
        <aside className="space-y-6">
          <section className="admin-panel"><h2 className="admin-panel-title"><ImageIcon size={20} /> Imagens</h2><label className="group relative mt-5 grid aspect-video w-full cursor-pointer place-items-center overflow-hidden rounded-xl border border-dashed border-gold/25 bg-gold/[0.03] text-xs text-white/35">{form.image && <Image src={form.image} alt={form.name || "Imagem do produto"} fill unoptimized={form.image.startsWith("http")} className="object-cover" sizes="320px" />}<span className={`relative z-10 rounded-full bg-black/70 px-4 py-2 ${form.image ? "opacity-0 transition group-hover:opacity-100" : ""}`}>{form.image ? "Substituir imagem" : "Enviar imagem principal"}</span><input type="file" accept="image/*" className="sr-only" onChange={(event) => void uploadProductImage(event.target.files?.[0])} /></label>{form.image && <p className="mt-3 truncate text-[0.62rem] text-white/25">{form.image}</p>}</section>
