  ...
  return (
    <AdminShell>
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_340px]">
        ...
        <aside className="space-y-6">
          <section className="admin-panel"><h2 className="admin-panel-title"><ImageIcon size={20} /> Imagens</h2><label className="mt-5 grid aspect-video w-full cursor-pointer place-items-center rounded-xl border border-dashed border-gold/25 bg-gold/[0.03] text-xs text-white/35">Enviar imagem principal<input type="file" accept="image/*" className="sr-only" onChange={(event) => void uploadProductImage(event.target.files?.[0])} /></label>{form.image && <p className="mt-3 truncate text-[0.62rem] text-white/25">{form.image}</p>}</section>
